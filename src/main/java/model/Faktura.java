...
public class Faktura {
    ...
    private static String generateRandom9Digits(){
        ...
        String randomNIP = String.valueOf(departmentCodes[ThreadLocalRandom.current().nextInt(0, departmentCodes.length)]);
        randomNIP += ThreadLocalRandom.current().nextInt(minNIPRandom,maxNIPRandom);
        return randomNIP;
    }
    public static String generateNIP(){
        String randomNIP;
        do {
            randomNIP = generateRandom9Digits();
            int controlSum = calculateControlNumber(randomNIP);
            randomNIP += controlSum;
        }while(randomNIP.length()>10);
        return randomNIP;
    }
    private static int calculateControlNumber(String NIP){
        int values[] ={ 6, 5, 7, 2, 3, 4, 5, 6, 7};
